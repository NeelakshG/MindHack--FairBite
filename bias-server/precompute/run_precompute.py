...
def compute_top_words(reviews_df: pd.DataFrame, top_n: int = TOP_N_WORDS) -> dict:
    """Extract top TF-IDF words per cuisine to show language differences."""
    # Balance review counts across cuisines so no cuisine dominates
    ...
    for cuisine, group in cuisine_groups.items():
        ...
        tfidf_matrix = vectorizer.fit_transform(sampled["text"])
